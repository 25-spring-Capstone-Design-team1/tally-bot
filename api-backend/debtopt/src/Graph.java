...
public class Graph {
    ...
    public Pair<Integer, List<ThreeTuple<Integer, Integer, Integer>>> eulerize() {
        ...
        return new Pair<>(startVertex, res);
    }
...
// 수정된 findEulerCircuit 메소드
    public ThreeTuple<List<Integer>, Map<Integer, Integer>, List<Integer>> findEulerCircuit(int startVertex) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (!tempAdjList.get(v).isEmpty()) {
                // 인접 간선 중 하나 선택
                Pair<Integer, Integer> entry = new Pair(tempAdjList.get(v).entrySet().iterator().next());
    ...
    public static List<FlattedGraph> removeAndSplit(FlattedGraph f2, InfiniteIterator<WeightStrategy> weightStrategies) {
        ...
        boolean isCycle = f.getCircuit().getFirst().equals(f.getCircuit().getLast());
        List<Integer> listToRemove = new ArrayList<>();

        List<FlattedGraph> res = new ArrayList<>();

        if (!isCycle) {
            f.getCircuit().addLast(f.getCircuit().getFirst());
            f.getWeights().addLast(0);
        ...
        List<Integer> newCircuit = new ArrayList<>(f.getCircuit().subList(listToRemove.getLast() + 1, f.getCircuit().size() - 1));
        newCircuit.addAll(f.getCircuit().subList(0, listToRemove.getFirst() + 1));
        List<Integer> newWeights = new ArrayList<>(f.getWeights().subList(listToRemove.getLast() + 1, f.getWeights().size()));
        newWeights.addAll(f.getWeights().subList(0, listToRemove.getFirst()));
        newWeights.replaceAll(w -> w - weightToRemove);
        Map<Integer, Integer> newWeightFrequency = new HashMap<>();
        for (Integer weight : newWeights) {
            newWeightFrequency.put(weight, newWeightFrequency.getOrDefault(weight, 0) + 1);
    ...
    public static Graph summarize(Graph g, InfiniteIterator<WeightStrategy> weightStrategies) {
        ...
        while(!q.isEmpty()) {
            ...
            List<ThreeTuple<Integer, Integer, Integer>> removedEdge = p.second();
            if(!p.second().isEmpty()) {
            ...
            ThreeTuple<List<Integer>, Map<Integer, Integer>, List<Integer>> res = t.findEulerCircuit(p.first());
    ...
    /*public static Graph summarize(Graph g, WeightStrategy weightStrategy) {
        List<Graph> l = UnionFind.splitGraph(g);
        return l.stream().map(h -> innerSummarize(h, weightStrategy)).reduce(new Graph(g.getVertexCount()), Graph::merge);
    }

    public static Graph innerSummarize(Graph t, WeightStrategy weightStrategy) {
        System.out.print(t);
        Graph t2 = new Graph(t);
        t = t.defaultGraph();
        if (t.getVertexCount() < 3 || t.getEdgeCount() < 2) {
            return t;
        }

        final int vertexCount = t.getVertexCount();
        t.integrateSameWeight();
        if (t.getVertexCount() < 3 || t.getEdgeCount() < 2) {
            return t;
        }
        Graph copy2 = new Graph(t);
        Pair<Integer, List<ThreeTuple<Integer, Integer, Integer>>> p = t.eulerize();
        List<ThreeTuple<Integer, Integer, Integer>> l = p.second();
        Graph resGraph = new Graph(t.getVertexCount());
        ThreeTuple<List<Integer>, Map<Integer, Integer>, List<Integer>> res = t.findEulerCircuit(p.first());
        Stream<FlattedGraph> stream = Stream.of(new FlattedGraph(true, res));
        Stream<Graph> resStream = stream.flatMap(f -> {
            return removeAndSplit(f, weightStrategy).map(e -> {
                Graph g = new Graph(vertexCount);
                g.addFlattedGraph(e);
                return g;
            });
        });
        Graph newGraph = resStream.reduce(resGraph, Graph::merge);
        for (ThreeTuple<Integer, Integer, Integer> e : l) {
            newGraph.addEdge(e.first(), e.second(), e.third());
        }
        if (copy2.getEdgeCount() < newGraph.getEdgeCount()) {
            newGraph = copy2;
        }
        if (newGraph.getVertexCount() < 3 || newGraph.getEdgeCount() < 2) {
            return t;
        }
        newGraph.integrateSameWeight();
        if (newGraph.getVertexCount() < 3 || newGraph.getEdgeCount() < 2) {
            return t;
        }
        newGraph.removeZero();
